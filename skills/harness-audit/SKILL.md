---
name: harness-audit
description: Guard smoke-test — red-team your own guardrails. Use periodically or after changing hooks/scripts/CI — "audit the harness", "are the guards still firing", "test the hooks". Re-tests every hook, git hook, gate, and script and reports any guard that has silently stopped firing.
---
...
1. **Harness hooks** (`.claude/hooks/` + settings wiring):
   ...
   - *check-agent-spawn* — attempt a builder spawn with no checkpoint file / no completion sentinel in the handoff; must be blocked.
   - *verify-green* (Stop hook) — introduce a deliberate lint error or artifact byte-drift in a scratch copy; the stop must be blocked.
2. **Git hooks** (`.githooks/`): pre-commit fires on a staged bad case (malformed agent frontmatter, script change without its tests, hand-edited generated artifact); verify `core.hooksPath` is actually configured — an unwired hook is a dead guard.
